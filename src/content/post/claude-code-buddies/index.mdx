...
You can swap your buddy by editing `~/.claude.json`. Change `oauthAccount.accountUuid` to any of the god-roll UUIDs above (or generate one with the picker below) and restart Claude Code. Since names and personalities are LLM-generated (not deterministic), they're stored in a separate `companion` section of the config that you can also customize:
...
Don't want a pre-rolled god tier? Pick your ideal species, eyes, hat, rarity, and shiny. We'll brute-force a random v4 UUID to find one that rolls your exact combo:
